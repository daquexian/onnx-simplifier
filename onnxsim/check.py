...
def add_all_features_to_output(m: onnx.ModelProto) -> None:
    ...
    for node in m.graph.node:
        for output in node.output:
            m.graph.output.extend([onnx.ValueInfoProto(name=output)])
add_all_features_to_output(model1)
...
# rand_input = generate_rand_input(model_opt, input_shapes=input_shapes)
  ...
  # }
# input = {'data': (np.arange(3*224*224).reshape((1, 3, 224, 224)).astype(np.float32) / (3*224*224 + 1) * 10000 + 1000).astype(np.float32)}
input = {'data': np.ones((1, 3, 224, 224), dtype=np.float32) * 100}

res1 = forward(model1, inputs=input)

# ok = check(model1, model2, 5)
print(res1['squeezenet0_conv0_fwd'])
print(res1['squeezenet0_pool0_fwd'])
print(res1['squeezenet0_flatten0_reshape0'])
